...
void discardSerialWhilePulseLocked();
...
/* =========================
SERIAL LOCK DURING PULSE MOVE
========================= */
void discardSerialWhilePulseLocked() {
if (!movingByPulses) return;

// While an encoder-based movement is active, ignore all incoming
// serial commands so nothing from Python can affect the current move.
clearSerialBuffers();
}
...
// Hard lock: while pulse motion is active, discard any serial input.
// This prevents Python commands like "servo 120" or "motor 200"
// from changing the ongoing encoder-based movement.
discardSerialWhilePulseLocked();

if (!movingByPulses && stringComplete) {
inputString.trim();
...
if (work == "left") {
  ...
  // This branch will practically never run now because movingByPulses
  // serial is discarded above, but it is kept here for completeness.
